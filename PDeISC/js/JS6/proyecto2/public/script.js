document.addEventListener('DOMContentLoaded', () => {
    ...
    const fileNameDisplay = document.getElementById('file-name-display'); // Nuevo elemento para mostrar el nombre del archivo
    ...
    inputArchivo.addEventListener('change', () => {
        if (inputArchivo.files.length > 0) {
            fileNameDisplay.textContent = inputArchivo.files[0].name;
            // Limpiar cualquier mensaje de error anterior sobre selección de archivo
            mensajeAnalisisErrorP.classList.add('hidden');
            mensajeAnalisisErrorP.textContent = '';
        } else {
    ...
    btnAnalizarArchivo.addEventListener('click', () => {
        ...
        if (!archivo) {
            ...
            // Oculta los resultados si no hay archivo
            resultadosAnalisisDiv.classList.add('hidden');
        ...
        if (!archivo.name.toLowerCase().endsWith('.txt')) {
            ...
            inputArchivo.value = ''; // Limpia el input de archivo
            fileNameDisplay.textContent = 'Ningún archivo seleccionado'; // Restablece el display
            resultadosAnalisisDiv.classList.add('hidden'); // Oculta los resultados
            btnDescargarFiltrados.classList.add('hidden');
            return;
        }
        ...
        lector.onload = (evento) => {
            ...
            lineas.forEach(linea => {
                ...
                // Si la línea está vacía o no es un número válido, la saltamos
            ...
            // Si no se procesó ningún número, muestra un mensaje y oculta los resultados
            if (totalNumeros === 0) {
                mostrarMensajeError('El archivo no contiene números válidos.', mensajeAnalisisErrorP);
                resultadosAnalisisDiv.classList.add('hidden');
                btnDescargarFiltrados.classList.add('hidden');
                return;
            }
        ...
        lector.onerror = () => {
            ...
            resultadosAnalisisDiv.classList.add('hidden'); // Oculta los resultados en caso de error
            btnDescargarFiltrados.classList.add('hidden');
        };
